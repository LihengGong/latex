...
\Procedure{PostOrder}{$t$} \Comment{$PostOrder$ receives vertex name as parameter}
  ...
  \EndFor
  \State print($t$) \Comment{Print $t$ upon exit}
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1]
\Procedure{Rotate}{$;;T,X$}\Comment{$T$\;and\;$X$\;are\;by\;reference}
  \State $OrigVal \gets T.val$
  \ForEach{child $w$ of $T$}
    \State Rotate($w, X$) \label{sec:TREEEVALUATION_2_4_b_L1}
  \EndFor
  \State $T.val \gets X$ \Comment{$X$ dumps its value to current node}
  \State $X \gets OrigVal$ \Comment{$X$ now carries current node's original value}
  \State print($T$)
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\Function{StkEvalList}{$L$}
  \While {L contains more than one element}
    ...
    \If{x is an operator}
      ...
      \State $result \gets eval(x, n1, n2)$ \Comment{apply operator $x$ to $n1$ and $n2$ and get the result}
